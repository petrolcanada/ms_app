  ...
  test('For any page number P and limit L, the API should return exactly L items (or fewer if on the last page)', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (page, limit) => {
          const response = await request(app).get('/api/funds').query({ page, limit });
          ...
          expect(response.body.meta.pagination).toHaveProperty('page', page);
          expect(response.body.meta.pagination).toHaveProperty('limit', limit);
